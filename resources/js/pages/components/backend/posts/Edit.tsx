...
interface Props {
    ...
    selected_category_tag_ids: number[];
    images: string[];
}
...
export default function PostEdit({
    ...
    post_types,
    category_tags,
    selected_category_tag_ids,
    images,
}: Props) {
    const { data, setData, post: submit, processing, errors } = useForm({
        _method: 'put',
        post_type_id: post.post_type_id ?? post_types?.[0]?.id ?? '',
        title: post.title ?? '',
        ...
        category_tag_ids: selected_category_tag_ids ?? [],
    });

    const [bannerPreview, setBannerPreview] = useState<string | null>(
        post.banner_img || null,
    );
    ...
    const onSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        submit('/admin/events-and-news/' + post.id);
    };

    return (
        <AppLayout breadcrumbs={[{ title: 'Event & News', href: '/admin/events-and-news' }, { title: 'Edit', href: '#' }]}>
            ...
            <div className="w-5xl mx-auto py-10 px-4">
                ...
                <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-sm border border-gray-100 dark:border-brand-core p-8">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Edit Post</h2>
                    ...
                    <form onSubmit={onSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="border-2 border-dashed rounded-xl p-6 text-center transition-colors border-gray-300 hover:bg-gray-50 dark:border-gray-600 dark:hover:bg-gray-700">
                                ...
                                <p className="text-xs text-gray-400 mt-2">Uploading new images will replace existing ones</p>
                            </div>
